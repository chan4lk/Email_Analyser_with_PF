...
important_keywords = ["urgent", "important", "critical"]
...
def generate_email_summary():
    ...
    for i, email_body in enumerate(email_bodies):
        ...
        is_important = any(keyword in plain_text.lower() for keyword in important_keywords)
        if is_important:
            feedback = "Important"
        else:
            feedback = "Not Important"

        summaries.append((summary, feedback))
        print(f"Summary for Email {i + 1}:")
        print(summary)
        print(f"Email {i + 1} is: {feedback}")
